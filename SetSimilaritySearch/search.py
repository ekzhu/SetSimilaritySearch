...
class SearchIndex(object):
    ...
    def __init__(self, sets, similarity_func_name="jaccard", similarity_threshold=0.5):
        ...
        self.overlap_threshold_func = _overlap_threshold_funcs[similarity_func_name]
        self.overlap_index_threshold_func = _overlap_index_threshold_funcs[
        ...
        logging.debug(
            "Finish frequency transform, {} tokens in total.".format(len(self.order))
        )
    ...
    def query(self, s):
        ...
        logging.debug(
            "{} original tokens and {} tokens after applying "
        ...
        candidates = set(
            [
                ...
                if self.position_filter_func(
                    l1, len(self.sets[i]), p1, p2, self.similarity_threshold
                )
            ]
        )
        ...
        for i in candidates:
            s2 = self.sets[i]
            sim = self.similarity_func(s1, s2, l1, len(s2))
