...
class TestSearchIndex(unittest.TestCase):
    ...
    def test_containment(self):
        ...
        # Threshold 0.1
        index = SearchIndex(
            sets, similarity_func_name="containment", similarity_threshold=0.1
        )
        results = index.query([3, 5, 4])
        correct_results = set(
            [(1, 1.0), (0, 1.0 / 3.0), (2, 2.0 / 3.0), (3, 1.0 / 3.0)]
        )
        self.assertEqual(set(results), correct_results)
        # Threshold 0.5
        index = SearchIndex(
            sets, similarity_func_name="containment", similarity_threshold=0.5
        )
        results = index.query([3, 5, 4])
